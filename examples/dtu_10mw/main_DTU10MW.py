...
def DTU10MW_Turbine():
    ...
    nearWakeLength = 359
    rotorRadius = 178.3 / 2.
    bLength = rotorRadius - hubRadius

    dataAirfoils = np.genfromtxt('../../turbineModels/DTU10MW/reference_files/dtu10mw.blade', skip_header=1, usecols=(7),
                                 dtype='U')
    intAirfoils = np.arange(0, len(dataAirfoils))

    data = np.genfromtxt('../../turbineModels/DTU10MW/reference_files/dtu10mw.blade', skip_header=1)

    refRadius = data[:,2] # np.linspace(0., bLength, 50)
    ...
    for i in range(len(refRadius)):
        ...
        centersAirfoils.append(Airfoil('../../turbineModels/DTU10MW/reference_files/' + foilName, headerLength=1))
...
def NewMexicoWindTurbine():
    ...
    refRadius = np.linspace(0., 2.04, 75) # data[:,2]
    ...
    for i in range(len(refRadius)):
        ...
        centersAirfoils.append(Airfoil('../turbineModels/NewMexico/reference_files/Airfoils/NACA64-418.foil', headerLength=1))
...
if(windTurbineCase == True):
    Blades, WindTurbine, uInfty, deltaFlts, deltaPtcles = DTU10MW_Turbine()
